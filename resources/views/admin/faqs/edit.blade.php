        ...
        <div class="card-body">
            <form action="{{ route('admin.faqs.update', $faq->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('short_answer') is-invalid @enderror" id="short_answer" name="short_answer" rows="5" required>{{ old('short_answer', $faq->short_answer) }}</textarea>
                    @error('short_answer')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('long_answer') is-invalid @enderror" id="long_answer" name="long_answer" rows="10">{{ old('long_answer', $faq->long_answer) }}</textarea>
                    @error('long_answer')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('is_show') is-invalid @enderror" id="is_show" name="is_show">
                        ...
                        <option value="1" {{ old('is_show', $faq->is_show ?? 0) == 1 ? 'selected' : '' }}>Yes</option>
                        <option value="0" {{ old('is_show', $faq->is_show ?? 0) == 0 ? 'selected' : '' }}>No</option>
                    </select>
                    @error('is_show')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
