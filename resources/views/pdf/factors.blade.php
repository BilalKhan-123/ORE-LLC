...
<body style="font-family: Arial, Helvetica, sans-serif;">
    <div class="container-fluid">
        <div class="row">
          <div class="col-lg-12">
            {{-- @dd($data['factors_explanations']['data']) --}}
                <h3 class="text-center f-21">Result Factors</h3> 
                @if($data && $data['factors_explanations'][0]->language == 'en')       
                    <div class="col-md-12 text-center font-weight-bold">
                        <h2 class="lg:block text-white ml-3">{{$data['factors_explanations'][0]->main_summary_text_for_table ?? ''}}</h2>
                        <h3 class="lg:block text-white ml-3">{{$data['factors_explanations'][0]->sub_summary_text_for_table ?? ''}}</h3>
                        <h5 class="lg:block text-white ml-3">{{$data['factors_explanations'][0]->personal_profile_summary ?? ''}}</h5>
                    </div>
                        ...
                        <table style="width:400px" class="table table-borderless font-weight-bold" border="1">
                            <thead>
                               <tr>
                                 ...
                                 <th class="py-2 px-5">Enhancer</th>
                               </tr>
                           </thead>
                           @php
                            ...
                            foreach ($testResults as $key => $item) {
                                ...
                                if (isset($data[$factor])) { // Check if the factor exists in the data array
                                    $testResults[$key]['data'] = [
                                        ...
                                        'percentile' => $data[$factor]['percentile'] ?? null                                       
                                    ];
                                }
                            }    
                           ...
                           @foreach($testResults as $key => $result)
                                @php
                                    $factor = strtolower($result['factor']); // Convert factor to lowercase
                                    $factor = str_replace(' ', '_', $factor); // Replace spaces with underscores
                                @endphp
                           <tr>
