...
<div class="container-fluid mt-4">
    <div class="row mb-4">
        <div class="col-md-8">
            {{-- <h1>{{ __('messages.edit_banner') ?? 'Edit Banner' }}</h1> --}}
            <h1>Edit Banner</h1>
        </div>
            ...
            <form action="{{ route('admin.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    @if($banner->image)
                        <div class="mt-2">
                            ...
                            <a target="__BLANK" class="glightbox" data-gallery="images-gallery" href="{{ asset('storage/'.$banner->main_image) }}"><img src="{{ asset('storage/'.$banner->main_image) }}" alt="{{ $banner->title }}" width="200" class="rounded d-block mt-1"></a>
                        </div>
                    @endif
                    @error('image')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('is_show') is-invalid @enderror" id="is_show" name="is_show">
                        ...
                        <option value="1" {{ old('is_show', $banner->is_show ?? 0) == 1 ? 'selected' : '' }}>Yes</option>
                        <option value="0" {{ old('is_show', $banner->is_show ?? 0) == 0 ? 'selected' : '' }}>No</option>
                    </select>
                    @error('is_show')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
